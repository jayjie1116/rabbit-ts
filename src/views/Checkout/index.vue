...
import { createOrderAPI, getCheckInfoAPI } from '@/api/checkout.ts';
import { useRouter } from 'vue-router';
import { useCartStore } from '@/stores/cartStore.ts';
import { Address, CheckInfoResult } from '@/api/model/checkoutModel.ts';

const router = useRouter();
const cartStore = useCartStore();
const checkInfo = ref({} as CheckInfoResult); // 订单对象
const curAddress = ref({} as Address); // 地址对象
const toggleFlag = ref(); // 控制显示地址切换弹窗
const addFlag = ref(); // 控制显示添加地址弹窗
const getCeckInfo = async () => {
  const res = await getCheckInfoAPI();
  checkInfo.value = res.result;
  // 筛选默认地址
  curAddress.value = checkInfo.value.userAddresses.find((item) => item.isDefault === 0) as Address;
